...
local function get_venv()
  ...
  local python_path = vim.lsp.get_active_clients({ name = 'pyright' })[1].config.settings.python.pythonPath
  if python_path == nil or python_path == '' then
    return '  Verify pythonPath in LSP'
  end
  local version = vim.fn.system { python_path, '--version' }
  version = version:gsub('[^A-Za-z0-9. ]+', '')
  version = version:gsub('[A-Za-z ]+', '')
  local path_str = dirname(python_path)
  if path_str ~= nil then
    path_str = path_str:gsub('[\\/]$', '')
    path_str = vim.fn.fnamemodify(path_str, ':~')
  end
  return version .. ' (' .. path_str .. ')'
  -- if string.find(filename, py_root, 1, true) ~= nil then
  --   print 'hooray!'
  --   return t
  -- elseif t ~= '' then
  --   print 'hooray kinda!'
